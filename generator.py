...
class Generator:
    ...
    def generate(self, file):
        random.seed()
        self.__puzzle = self.__generate_solved_puzzle()
        if file is None:
    ...
    def __generate_solved_puzzle(self):
        ...
        while k < m and l < n:
            for i in range(l, n):
                puzzle[k][i] = self.__puzzle_numbers[val]
                val += 1
            k += 1

            for i in range(k, m):
                puzzle[i][n - 1] = self.__puzzle_numbers[val]
                val += 1
            n -= 1

            if k < m:
                for i in range(n - 1, (l - 1), -1):
                    puzzle[m - 1][i] = self.__puzzle_numbers[val]
                    val += 1
                m -= 1

            if l < n:
                for i in range(m - 1, k - 1, -1):
                    puzzle[i][l] = self.__puzzle_numbers[val]
                    val += 1
                l += 1
        return puzzle
